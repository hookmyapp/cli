...
describe("agent setup (AIT-468)", () => {
  it("hands Cursor and Codex a credential /mcp accepts, and not the session JWT", async () => {
    ...
    // skip the logout and strand a live key on the shared test org.
    try {
      const sessionToken = JSON.parse(
        await readFile(
          path.join(cliConfigDir(session.home), "credentials.json"),
          "utf8",
        ),
      ).accessToken as string;

      const setup = await runCli(
        ["agent", "setup", "--client", "cursor", "--no-skills"],
        {
          home: session.home,
        },
      );
      ...
      const cursor = JSON.parse(
        ...
        mcpServers: {
          hookmyapp: { url: string; headers: { Authorization: string } };
        };
      };
      ...
      const helper = await runCli(["mcp-headers", "--header", "x-api-key"], {
        home: session.home,
      });
      expect(helper.exitCode).toBe(0);
      const codexToken = (JSON.parse(helper.stdout) as Record<string, string>)[
        "X-API-Key"
      ];
      ...
      expect(await callMcp({ Authorization: `Bearer ${cursorToken}` })).toBe(
        200,
      );
      expect(await callMcp({ "X-API-Key": codexToken })).toBe(200);
      // The whole reason the mint exists: this token has no `aud` claim.
      expect(await callMcp({ Authorization: `Bearer ${sessionToken}` })).toBe(
        401,
      );
    } finally {
      // logout revokes this machine's keys by name, so it sweeps a key a
