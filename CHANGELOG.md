...
- `hookmyapp billing` shows the new plans. Trial organizations see days left and actions used so far (unlimited during the trial), with a reminder to add a card. Build, Scale, and Business organizations see their action count against the plan quota, with a next-plan hint when usage runs hot. Organizations on existing plans see the same output as before (AIT-420).
