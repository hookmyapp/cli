...
- A sign-in service refusal, a malformed credential response and a non-401 refusal of the log stream were reported as `AUTH_REQUIRED`; they now carry their own codes (`WORKOS_DEVICE_AUTH_FAILED`, `WORKOS_DEVICE_AUTH_MALFORMED`, `WORKOS_DEVICE_TOKEN_FAILED`, `WORKOS_DEVICE_TOKEN_MALFORMED`, `AGENT_CREDENTIAL_MALFORMED`, `SSE_CONNECT_FAILED`) as sev2, exit code still 4; a declined or lapsed device code is still `AUTH_REQUIRED`. A media node without a url, an empty paid-plan catalog, an empty scope discovery, missing billing eligibility and a non-public workspace id on disk (`NO_MEDIA_URL`, `PLANS_EMPTY`, `SCOPES_UNRESOLVED`, `ELIGIBILITY_UNAVAILABLE`, `WORKSPACE_ID_NOT_PUBLIC_ID`, `WORKSPACE_ORG_MISSING`) are sev2 too, exit code still 2. Workspace discovery no longer turns a server 5xx into "workspace not found". A token refresh that succeeds but cannot write the new token to a read-only config directory now surfaces `CONFIG_WRITE_FORBIDDEN` instead of "Session expired"; any other failure to save it is `CREDENTIAL_WRITE_FAILED`. A log-stream connection that fails at the transport level is a `NETWORK_ERROR` (exit 5) like every other connection failure (AIT-652).
