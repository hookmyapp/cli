...
- `facebook publish --video` prints the `{pageId}_{videoId}` post id (the id `comments`, `insights` and `delete-post` take) instead of the bare video id; `--reel` answers with the same post id and the video id (AIT-621).
