...
All existing ~11 subcommands (`list`, `show`, `connect`, `disconnect`, `enable`, `disable`, `env`, `token`, `health`, `webhook show/set`, `channels listen`, `channels logs`) work with Instagram channels in v1. Same flag surface, same exit codes, same JSON shape per type. No new subcommands; in particular, **no `channels webhook clear` / `channels webhook remove`** — that's a sandbox-only concept (sandbox `clear` reverts the session to the CLI tunnel default; real channels have no such default to revert to). Customers who want to halt forwarding on a real channel use `channels disable` (which stops the forwarder without unsetting the URL); customers who want to redirect forwarding use `channels webhook set --url <new>`. There is intentionally no "unset webhook URL to null" operation in this milestone — the current CLI's `runChannelWebhookSet` already rejects falsy `--url` values, and exposing a `null`-write would force a backend semantics decision (does Meta receive a 200 with no forward, or does forwarding silently break?) that's out of scope.
...
- **Non-TTY:** there is no non-TTY connect path. D6 establishes that `channels connect` (any type, bare or explicit) refuses non-TTY because OAuth requires a browser. So the only way `connect` reaches the backend is from a TTY — where bare connect prompts and explicit connect runs directly. Non-TTY connect always exits 2.
- **Login `--next channels` wizard:** the wizard becomes channel-type-aware in TTY only. After successful login, if `--next channels` is set, it calls the same `@inquirer/select` channel-type picker before invoking connect. The wizard never silently defaults to whatsapp — that would re-introduce the "IG is second-class" surface bug we're fixing. In non-TTY environments `--next channels` exits 2 via the D6 refusal, same as bare `channels connect`.
