...
describe('displayEmail', () => {
  test('masks local part after 2 chars and domain to first char + tld', () => {
    expect(displayEmail('info@octavia.com')).toBe('in***@o***.com');
    expect(displayEmail('edmond55@gmail.com')).toBe('ed***@g***.com');
  });

  test('never emits the raw address', () => {
    for (const raw of ['info@octavia.com', 'edmond55@gmail.com']) {
