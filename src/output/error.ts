...
//   1. `err instanceof CliError` — an earlier revision code paths (sandbox-listen/picker,
//      sandbox-listen/binary, index.ts main(), api-403-handler.spec.ts) keep
//      working.
//   2. `err instanceof AppError` — new an earlier revision code + Sentry `captureError`
...
//
// an earlier revision exit-code contract:
//   AuthError        → 4
//   PermissionError  → 3
//   ValidationError  → 2
//   ConflictError    → 6
...
/**
 * an earlier revision legacy base class. Now a concrete subclass of `AppError` so that:
 *
...
/**
 * Map any thrown value to a CLI exit code. an earlier revision's exit-code contract is
...
export function exitCodeFor(err: unknown): number {
  ...
  if (err instanceof CliError) {
    ...
    // an earlier revision bootstrap-code API errors set this explicitly).
