...
describe('parseIdentifier — shape detection', () => {
  it('+E164 phone → { kind: "phone", value: digits-only }', () => {
    expect(parseIdentifier('+14155552671')).toEqual({ kind: 'phone', value: '14155552671' });
  ...
  it('bare digits without + → ValidationError with phone suggestion', () => {
    expect(() => parseIdentifier('14155552671')).toThrow(ValidationError);
    expect(() => parseIdentifier('14155552671')).toThrow(/Did you mean \+14155552671/);
  });
