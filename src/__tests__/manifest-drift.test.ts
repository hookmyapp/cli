...
describe('CLI events.manifest.json structural contract', () => {
  ...
  it('contains exactly 11 events (CLI-emitted scope only)', () => {
    expect(manifest.events).toHaveLength(11);
  });
...
describe('CLI manifest ↔ runtime registry drift', () => {
  it('runtime registry has exactly 11 names (CLI-emitted scope only)', () => {
    expect(EVENT_NAMES_RUNTIME).toHaveLength(11);
  });
